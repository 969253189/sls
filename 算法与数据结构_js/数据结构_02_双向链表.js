...
class LinkedList {
   ...
   * @return Object
   * */
  _Node(element, next = null, previous = null) {
    this.length++;
    return {element, next, previous}
  }
  ...
  append(element) {
    if (this.lastElement && this.lastElement instanceof Object) {
      this.lastElement.next = this._Node(element, null, this.lastElement);
  ...
  insert(index, element) {
    ...
    newElement.previous = preElement;
...
for (let index = 0; index <= 10000000; index++) linkedListTest.append(index);
// linkedListTest.data = linkedListTest.reverse(linkedListTest.data);
console.log(new Date().getTime(), JSON.stringify(linkedListTest));